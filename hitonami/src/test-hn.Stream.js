...
function test_VbTSQqsN(streamFactory,pairList,s){
	...
	for(var pairListIdx=0;pairListIdx<pairList.length;++pairListIdx){
		...
		if(start<=dataSize){
			ut.eqq("mNehiCRh",stream.seek(start,hn.Stream.SEEK_SET),0);
			ut.eqq("EoeSLkzB",stream.tell(),start);
			var data0 = stream.read(len0);

			ut.eqq("MqLlWmnL",data0.getSize(),len1);
			ut.eqq("VxhBnWnJ",stream.tell(),start+len1);
			
			var data1 = data.mid(start,len1);
			ut.eqq("auRwxbuH",data1.getSize(),len1);
			ut.tt("YWMeZBzK "+s+" "+start,data0.equal(data1));
		}else{
			ut.eqq("VyrAAmZM "+s+" "+start,stream.seek(start,hn.Stream.SEEK_SET),-1);
		}
	}
...
function test_YJZPAOpk(streamFactory,offsetListList,s){
	...
	for(var offset_list_list_idx=0;offset_list_list_idx<offset_list_list.length;++offset_list_list_idx){
		...
		while(offset_idx<offset_list.length){
			...
			ut.tt("jHtHdjHt "+offset_list[0]+" "+buf_start+" "+buf_end,true);
			
			if(buf_start>dataSize){
				ut.eqq("YYbkCGcF "+s,stream.seek(skip_len,hn.Stream.SEEK_CUR),-1);
				break;
			}
			ut.eqq("YYbkCGcF",stream.seek(skip_len,hn.Stream.SEEK_CUR),0);
			ut.eqq("xfMqDNai",stream.tell(),buf_start);
			var data0 = stream.read(read_len);
			ut.eqq("IihrHWgk",data0.getSize(),expected_read_len);
			ut.eqq("QCAhHHMN",stream.tell(),expected_tell);

			var data1 = data.mid(buf_start,read_len);
			ut.eqq("FYyXdUbd",data1.getSize(),expected_read_len);
			ut.tt("ZHBZpnko",data0.equal(data1));
...
function test_qbrFviLR(streamFactory,s){
	...
	ut.tt("kHEZAYzl "+s, stream.open());

	ut.eqq("EwkgxoWA "+s,stream.seek(0,hn.Stream.SEEK_END),0);
	ut.eqq("eLYtPWsB "+s,stream.tell(),dataSize);
...
function test_CSHGEYOR(streamFactory,opList,s){
	...
	for(var i in opList){
		var op=opList[i];
		if(op["op"]=="open"){
			ut.tt("XIFGOMJH",stream.open());
			offset=0;
		}else if(op["op"]=="seek"){
			ut.eqq("ZSSUSCAV",stream.seek(op["offset"],op["type"]),0);
			if(op["type"]==hn.Stream.SEEK_SET)offset=0;
			else if(op["type"]==hn.Stream.SEEK_END)offset=dataSize;
			offset+=op["offset"];
		}else if(op["op"]=="tell"){
			ut.eqq("XDACYPRS",stream.tell(),offset);
		}else if(op["op"]=="read"){
			var tmp=stream.read(op["len"]);
			ut.eqq("VYDRTSJH",tmp.getSize(),op["len"]);
			ut.tt("HHOPVJWD",tmp.equal(data.mid(offset,op["len"])));
			offset+=op["len"];
		}else if(op["op"]=="close"){
			stream.close();
		}else{
